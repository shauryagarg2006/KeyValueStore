...
public class ObjectServer implements ObjectStore {
	...
	/*
	 * Reference to underlying chord node - only if client wants to join as a
	 * node
	 */
	NodeOperations node = null;

	/*
	 * Reference to remote chord nodes Object store - only if client refuses to
	 * join as a node
	 */
	...
	public ObjectServer(ArrayList<InetAddress> bootstrapNodes, boolean joinNetwork) {
		try {
	...
	private NodeOperations joinNetwork(ArrayList<InetAddress> bootstrapNodeList) throws RemoteException {
		Node n = new ChordDriver.NodeBuilder().setRegistryManager(new RegistryManagerImpl())
				.setBootStrapNodes(bootstrapNodeList).setCHORD_ID_MAX_BITS(10).setSUCCESSOR_LIST_MAX_SIZE(3).Build();
